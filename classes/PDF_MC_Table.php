
<?php
require_once('class_tFPDF/tfpdf.php');

class PDF_MC_Table extends tFPDF
{
var $widths;
var $aligns;

function SetWidths($w)
{
    //Set the array of column widths
    $this->widths=$w;
}

function SetAligns($a)
{
    //Set the array of column alignments
    $this->aligns=$a;
}

function Row($data)
{
    //Calculate the height of the row
    $nb=0;
    for($i=0;$i<count($data);$i++) 
        $nb=max($nb,$this->NbLines($this->widths[$i],$data[$i]));        
    $h=5*$nb;
    if($h < 15) {
        $h = 15;
    }    
    //Issue a page break first if needed
    $this->CheckPageBreak($h);
    //Draw the cells of the row   
    $x=$this->GetX();
    $y=$this->GetY();
    for($i=0;$i<count($data);$i++)
    {
        $w=$this->widths[$i];
        $a=isset($this->aligns[$i]) ? $this->aligns[$i] : 'R';       

        if(gettype($data[$i]) === 'integer' || gettype($data[$i]) === 'double') {
            $a = 'R';
        } else if(gettype($data[$i]) === 'double') {
            $a = 'R';
        } else if(gettype($data[$i]) === 'string') {
            $a = 'L';
        } else {
            $a = 'C';
        }
        //Save the current position
        $x=$this->GetX();
        $y=$this->GetY();
        //Draw the border
        $this->Rect($x,$y,$w,$h);
        //Print the text
        if($i != (count($data) - 1)) {
            $this->MultiCell($w,5,$data[$i],0,$a);
        } else {
            $this->Cell($w,15," ",'T',0,'C'); 
            if(isset($data[$i]) && $data[$i] !== 'undefined') {
                $this->Image($data[$i],267,$y,15,15);
            }           
        }        
        //Put the position to the right of the cell
        $this->SetXY($x+$w,$y);
    }    
    //Go to the next line
    $this->Ln($h);
}

function viewHeader($w,$header)
{
    // Header
    $this->SetFontSize(15);
    $this->Cell(array_sum($w),10,'Ведомость осветительных приборов',0,0,'C');   
    $this->Ln();
    // Colors, line width and bold font
    $this->SetFontSize(10);
    $this->SetFillColor(236,125,99);
    $this->SetTextColor(255);
    $this->SetDrawColor(236,125,99);
    $this->SetLineWidth(.3);
    for($i=0;$i<count($header);$i++) {          
        $this->Cell($w[$i],7,$header[$i],1,0,'C',1);
    }
    $this->Ln();
}

function CheckPageBreak($h)
{
    //If the height h would cause an overflow, add a new page immediately
    if($this->GetY()+$h>$this->PageBreakTrigger)
        $this->AddPage($this->CurOrientation);
}

    function NbLines($w,$txt)
    {
        //Computes the number of lines a MultiCell of width w will take
        $cw=&$this->CurrentFont['cw'];        
        if($w==0)
            $w=$this->w-$this->rMargin-$this->x;
        $wmax=($w-2*$this->cMargin)*1000/$this->FontSize;       
        $s=str_replace("\r",'',$txt);
        $nb=strlen($s);
        if($nb>0 and $s[$nb-1]=="\n")
            $nb--;
        $sep=-1;
        $i=0;
        $j=0;
        $l=0;
        $nl=1;
        while($i<$nb)
        {
            $c=$s[$i];
            if($c=="\n")
            {
                $i++;
                $sep=-1;
                $j=$i;
                $l=0;
                $nl++;
                continue;
            }
            if($c==' ') {
                $sep=$i;
            }        

            /* $l+=$cw[$c];  */       
             $l+=556; 
                     
            if($l>$wmax)
            {             
                if($sep==-1)
                {
                    if($i==$j)
                        $i++;
                }
                else
                    $i=$sep+1;
                $sep=-1;
                $j=$i;
                $l=0;
                $nl++;
            }
            else
                $i++;
        }
      
        return $nl;
    }    
}
?>